...
//        builder.append("UDID: ").append(udid).append("\n");
    ...
    private void callBack(String start, String msg, String fails) {
        if (_listener != null) {
            if (!start.equals(""))
                _listener.OnInitStart();
            else if (!fails.equals(""))
                _listener.OnInitFail(fails);
            else
                _listener.OnIdsAvalid(msg);
        }
    }
